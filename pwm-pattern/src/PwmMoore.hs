{- |
PWM pattern generator on the C5G, built with the __Moore__ construction.
...
by 'runMoore' (Clash's 'Clash.Prelude.moore'); the sibling "PwmMealy" is identical
but for swapping 'runMoore' → 'runMealy', which is the whole point of the example.
...
import PwmPattern.Pattern (PrescaleExp, initial, prescale, runMoore)
...
hidden clock/reset/enable that 'pwm', 'prescale' and 'runMoore' need.
-}
topEntity ::
...
patternLed sw = led
    where
        ...
        dutyConst = runMoore (initial :: Constant) tick
        dutyTri = runMoore (initial :: Triangle) tick
