# Quartus project-generation script for the C5G pwm-pattern example.
#
# Run by the Makefile as:  quartus_sh -t <abs path>/pwm-pattern.tcl <NAME>
# with the current working directory set to _build/<NAME>/02-quartus/ (Quartus
# CLI tools are cwd-oriented). It (re)creates the `pwm-pattern` project there,
# writing device, source, timing, and pin assignments into pwm-pattern.qsf. The
# discrete compile stages (quartus_map/fit/asm/sta) are then driven from the
# Makefile, mirroring the reference repo's yosys/nextpnr/icepack/iceprog split.
#
...
#
# Both tops (PwmMoore and PwmMealy) expose the SAME ports (clk/sw/led), so unlike
# blinky there are no design-specific pins; the same assignments serve both.

package require ::quartus::project

# -overwrite: re-run from a clean slate each build; the Makefile owns staleness.
# The project name must match the Makefile's QPROJ (pwm-pattern) so the discrete
# stages (quartus_map pwm-pattern, ...) find the revision they expect.
project_new pwm-pattern -overwrite
...
# board labels (CLOCK_50_B5B, LEDR[0], SW[0]) are irrelevant to Clash; only the
...
#   sw  -> SW[0]        -> PIN_AC9, 1.2 V          (pattern select slide switch)
...
#   }
#
# The remaining slide switches SW[9:1] (1.2 V) and push buttons KEY[3:0] (1.2 V,
# active-low) attach the same way when needed — e.g. more pattern-select bits.
